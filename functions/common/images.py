...
def nearest_uv_coord(loc, img_obj):
    obj_width = img_obj.empty_display_size
    size = Vector(img_obj.data.size)
    off = Vector(img_obj.empty_image_offset)
    relative_loc = loc.xy - img_obj.location.xy
    pixel_offset = relative_loc * (size.x / obj_width)
    pixel_loc = -vec_mult(size, off) + Vector(pixel_offset[:2])
    return pixel_loc
